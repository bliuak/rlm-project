...
DEFAULT_EXISTING_ANSWER = (
    Path(__file__).resolve().parents[1] / "results" / "oolong_pairs_verified_answers" / "paper_01.txt"
)
...
def user_satisfies_paper_01(records: list[dict[str, Any]]) -> bool:
...
def user_audit(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "has_numeric_value_or_location": bool(target_entries),
        "qualifies": user_satisfies_paper_01(records),
...
def expected_pairs(records_by_user: dict[int, list[dict[str, Any]]]) -> list[tuple[int, int]]:
    qualifying_users = [
        user for user, records in records_by_user.items() if user_satisfies_paper_01(records)
    ]
...
def build_audit(items: list[dict[str, Any]], compare_to: Path | None) -> dict[str, Any]:
    ...
    return {
        "task": "paper_01",
        "criterion": "Both users have at least one instance with a numeric value or location.",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate the paper_01 pair answer from a standalone criterion encoding.")
...
def main() -> None:
    ...
    print(f"paper_01 pair_count={audit['pair_count']}")
