...
# Task 13: one user has exactly one description; the other has abbreviation and entity.
...
def first_condition(counts: Counter[str]) -> bool:
    return counts["description and abstract concept"] == 1


def second_condition(counts: Counter[str]) -> bool:
    return counts["abbreviation"] >= 1 and counts["entity"] >= 1
