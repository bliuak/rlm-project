...
# Task 14: one user has human being and numeric value; the other has exactly two locations.
...
def first_condition(counts: Counter[str]) -> bool:
    return counts["human being"] >= 1 and counts["numeric value"] >= 1


def second_condition(counts: Counter[str]) -> bool:
    return counts["location"] == 2
