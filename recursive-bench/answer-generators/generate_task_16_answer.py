...
# Task 16: one user has description and human being; the other has at least two entities and one abbreviation.
...
def first_condition(counts: Counter[str]) -> bool:
    return counts["description and abstract concept"] >= 1 and counts["human being"] >= 1


def second_condition(counts: Counter[str]) -> bool:
    return counts["entity"] >= 2 and counts["abbreviation"] == 1
