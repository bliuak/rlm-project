...
# Task 12: one user has at least two numeric values; the other has location and human being.
...
def first_condition(counts: Counter[str]) -> bool:
    return counts["numeric value"] >= 2


def second_condition(counts: Counter[str]) -> bool:
    return counts["location"] >= 1 and counts["human being"] >= 1
