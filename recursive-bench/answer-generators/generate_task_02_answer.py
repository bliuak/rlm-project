...
DEFAULT_EXISTING_ANSWER = (
    PROJECT_ROOT / "results" / "oolong_pairs_verified_answers" / "task_02.txt"
)
...
def user_satisfies_task_02(records: list[dict[str, Any]]) -> bool:
...
def user_audit(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "qualifies": user_satisfies_task_02(records),
...
def expected_pairs(records_by_user: dict[int, list[dict[str, Any]]]) -> list[tuple[int, int]]:
    qualifying_users = [
        user for user, records in records_by_user.items() if user_satisfies_task_02(records)
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate the task_02 pair answer from a standalone criterion encoding.")
...
def main() -> None:
    ...
    print(f"task_02 pair_count={audit['pair_count']}")
